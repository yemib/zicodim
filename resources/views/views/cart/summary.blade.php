...
$products  = add_product::find($checking_carts->product_id) ;
		...
		@endif
	<strong  class="price_size">SubTotal : <span class=""  >  &#x20A6; </span> <span class=""  id="subtotal"> {{ number_format(  $subtotal1  ) }}   </span>  
	 ...
	 <input type="hidden"  id="subtotalt"  value="{{ $subtotal1  }}"    />
			...
			<strong  class="price_size">Delivery Charges  :  <span > &#x20A6;  </span>  <span   id="delivery_price_here"> {{ number_format(  $delivery1) }}  </span> </strong>
			
			
			
			<hr/>
			
			<strong  class="price_size">Total   : &#x20A6;<span  id="total_value2">  {{  number_format(  $subtotal1    +  $delivery1 ) }}   </span>  </strong>  
